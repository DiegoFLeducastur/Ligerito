...
export default function Sidebar({ listas = [], idListaActiva, onSeleccionarLista, onCrearLista, onBorrarLista, inventario = [], onAñadirAlInventario }) {
  ...
  const [itemSeleccionado, setItemSeleccionado] = useState(null); // Para saber qué ítem abrió el menú
    ...
    <aside className="w-84 bg-slate-800 text-white h-screen flex flex-col shrink-0 shadow-2xl overflow-hidden border-r border-slate-700/50" role="complementary">
      ...
      <div className="px-5 flex-1 flex flex-col min-h-0">
        ...
        <div className="flex-1 overflow-y-auto space-y-1 custom-scrollbar pr-1 pb-6 -mx-1">
          {filtrados.map((obj) => {
            // Buscamos las categorías de la mochila activa para mostrarlas en el menú
            const mochilaActual = listas.find(l => l.id === idListaActiva);
            const categoriasDisponibles = mochilaActual?.categorias || [];

            return (
              <div key={obj.id} className="flex flex-col p-2 px-3 rounded-xl bg-white/5 border border-white/5 group gap-2 transition-all">
                <div className="flex items-center justify-between gap-3">
                  ...
                  {/* Cambiamos el comportamiento: ahora alterna el menú */}
                  <button
                    onClick={() => setItemAbierto(itemAbierto === obj.id ? null : obj.id)}
                    className={`transition-colors cursor-pointer ${itemAbierto === obj.id ? 'text-blue-400' : 'text-slate-400 hover:text-blue-400'}`}
                  >
                ...
                {itemAbierto === obj.id && (
                  <div className="bg-slate-900/80 rounded-lg p-2 border border-slate-700 space-y-1 animate-in slide-in-from-top-1 duration-200">
                    <p className="text-[9px] uppercase font-bold text-slate-500 px-1 mb-1">Selecciona Categoría:</p>
                        ...
                        {categoriasDisponibles.map(cat => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              setItemAbierto(null); // Cerramos al añadir
                            }}
